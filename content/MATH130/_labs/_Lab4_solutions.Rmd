...
```{r}
a <- select(hflights, starts_with("D"))
names(a)
```

2. All variables that end with `Num`.
```{r}
a <- select(hflights, ends_with("Num"))
names(a)
```

3. All variables that contain an the word `Taxi` or `Delay`.  
```{r}
a <- select(hflights, contains("Taxi"), contains("Delay"))
names(a)
```
...
Are there longer delays on rainy days? Follow these steps to answer the question. At each step do the following: 
* save the resulting code to a data frame (object)
* print the top (`head`) of that object to the report
...
```{r}
days_with_rain <- weather %>% 
                    group_by(year, month, day) %>%
                    summarise(daily_precip = sum(precip)) %>%
                    mutate(rainday = ifelse(daily_precip > 1, "Rainy", "No Rain"))
head(days_with_rain)
```
...
```{r}
flights_with_raindata <- left_join(flights, days_with_rain) %>% 
                            select(dep_delay, rainday)
head(flights_with_raindata)
```

3. Examine the distribution of the departure delays across rainy and non-rainy days.
```{r}
ggplot(flights_with_raindata, aes(y=dep_delay, x=rainday)) + geom_boxplot()
```

4. Explain why there is a NA category.

Some records are missing values for `dep_delay` or `rainday`. 
```{r}
table(is.na(flights_with_raindata$dep_delay))
```
...
```{r}
flights_nomiss <- na.omit(flights_with_raindata)
```
...
```{r}
NROW(flights_with_raindata) - NROW(flights_nomiss)
```


7. Recreate your plot using the data without missing values and answer the original question
```{r}
ggplot(flights_nomiss, aes(y=dep_delay, x=rainday)) + geom_boxplot()
```
