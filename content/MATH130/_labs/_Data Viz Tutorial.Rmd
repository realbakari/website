...
library(knitr); library(gridExtra); library(ggplot2)
...
We will also use the two most common methods used to create plots. 1) Base graphics, 2) the `ggplot2` package. Each have their own advantages and disadvantges. If you have not done so already, go ahead and install the `ggplot2` package now. 
...
2. FYEO - using `ggplot2`. Each have their own advantages and disadvantges. 
...
## Plot
The most basic plotting function is simply `plot()`. This function gives you a plot suitable for the data type you give it. Here `depth` is a continuous variable, so `plot()` returns a dotplot of the value of depth against the row number (index).
```{r}
plot(dsmall$depth)
```

Where here, `plot()` returns a barchart when I provide a categorical variable `carat` as the variable to plot. 
```{r}
plot(dsmall$carat)
```
...
### Pie Chart
Pie charts are my _least_ favorite plotting type. Human eyeballs can't distinguish between angles as well as we can with heights. Just like `barplot()`, `pie()` takes a table object as it's argument. 
...
There are ways to make a nice one using ggplot, but i'm more familiar with base graphics here. So that's whatcha get. The primary piece needed is to add the percentages of each wedge. 
```{r}
...
#### ggplot
qq (or qnorm) plots specifically plot the data against a theoretical distribution. That means in the `aes()` aestetic argument we don't specify either x or y, but instead the `sample=` is the variable we want to plot. 
```{r}
...
Stacked bars can be difficult to interpret, and very difficult to compare values between groups. A side by side barchart is preferrable. 
...
## Scatterplot matrix
A scatterplot matrix allows you to look at the bivariate comparison of multiple pairs of variables simultaneously. First we need to trim down the data set to only include the variables we want to plot, then we use teh `pairs()` function.
...
get the average price per combiation of carat and cut. 
...
knowledge of the grahpics system to do so. So I do not show it here. 
...
#### ggplot
We can reproduce the same thing by plotting one continous variable against one categorical variable, and adding a layer of points. I'd argue that horizontal looks better due to the axis-labels. 
```{r}
...
# Joy plots
Hot off the press, gaining instant popularity this summer. They are a combination of a Cleveland dot plot and a density plot. 
...
It is important to compare distriubtions across groups on the same scale, and our eyes can compare items vertically better than horizontally. So let's force `ncol=1`. 
...
Who says we're stuck with only faceting on one variable? A varient on `facet_wrap` is `facet_grid`. Here we can specify multiple variables to panel on. 
...
Go here http://www.cookbook-r.com/Graphs/ for these. This tutorial is long enough. 
